...
int main(int argc, char *argv[])
{
    ...
    Genome newPop[POP_SIZE];
    Genome winner(goal);
    winner.calculateFitness(goal);

    for (int generation = 0; generation < GENERATIONS; generation++)
    {
        sort(begin(population), end(population), [=](Genome a, Genome b) -> bool
        ...
        for (int i = 0; i < POP_SIZE; i++)
        {
            ...
            Genome testGenome = population[first].crossover(population[second]).mutation();
